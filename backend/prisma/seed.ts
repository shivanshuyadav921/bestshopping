...
async function main() {
  ...
  // Create explicit admin user requested by USER
  const shivaPasswordHash = await bcrypt.hash("shiva@123", 10);
  const shivaAdmin = await prisma.user.upsert({
    where: { email: "shivanshuy921@gmail.com" },
    update: { name: "Shiva Admin", role: UserRole.ADMIN, passwordHash: shivaPasswordHash },
    create: { email: "shivanshuy921@gmail.com", name: "Shiva Admin", role: UserRole.ADMIN, passwordHash: shivaPasswordHash },
  });
  dbUsers.push(shivaAdmin);
