    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12 sm:px-6 lg:px-8 relative">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10 border border-white/10 bg-card p-8 md:p-10 shadow-2xl">
        ...
        <div className="border-t border-white/10 pt-6 space-y-4">
          <p className="text-[10px] font-bold tracking-widest text-center text-white/40 uppercase">
            Development Quick Access
          </p>
          <div className="grid grid-cols-3 gap-2">
            <button
              onClick={() => handleQuickLogin('customer@client.com')}
              className="py-2 border border-white/5 bg-white/[0.02] text-[10px] font-mono text-white/70 hover:border-accent/40 hover:text-white"
            >
              CUSTOMER
            </button>
            <button
              onClick={() => handleQuickLogin('admin@prema.com')}
              className="py-2 border border-white/5 bg-white/[0.02] text-[10px] font-mono text-white/70 hover:border-accent/40 hover:text-white"
            >
              ADMIN
            </button>
            <button
              onClick={() => handleQuickLogin('owner@prema.com')}
              className="py-2 border border-white/5 bg-white/[0.02] text-[10px] font-mono text-white/70 hover:border-accent/40 hover:text-white"
            >
              OWNER
            </button>
