...
/*                                                                          */
/* This is a self-contained, front-end intake wizard. On submit, it       */
/* registers uploaded files via POST ${process.env.NEXT_PUBLIC_API_URL || ""}/api/files and submits the RFQ via   */
/* POST ${process.env.NEXT_PUBLIC_API_URL || ""}/api/customer/rfq. All data persists in PostgreSQL via Prisma.    */
...
function SelectionCard({
  ...
  return (
    <button
      ...
      className={`relative text-left p-5 border bg-background transition-all duration-200 group focus:outline-none focus:ring-1 focus:ring-accent/80 focus:border-accent ${
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const file of form.files) {
        ...
        const registerRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/files`, {
          method: 'POST',
      ...
      const rfqRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/customer/rfq`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          description: desc,
              ...
              >
                ...
                {step === 0 && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="radiogroup" aria-label="What You Have">
                ...
                {step === 2 && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4" role="radiogroup" aria-label="Component Category">
                      {CATEGORIES.map((opt) => (
                        <SelectionCard
                          key={opt.id}
                          option={opt}
                ...
                {step === 3 && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4" role="radiogroup" aria-label="Material Selection">
                      {MATERIALS.map((opt) => (
                        <SelectionCard
                          key={opt.id}
                          option={opt}
                ...
                {step === 4 && (
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2" role="radiogroup" aria-label="Required Tolerance">
                        {TOLERANCES.map((t) => (
                          <button
                            key={t}
                            type="button"
                            role="radio"
                            aria-checked={form.tolerance === t}
                            onClick={() => update('tolerance', t)}
                            className={`px-4 py-2 text-sm border transition-colors duration-200 focus:outline-none focus:ring-1 focus:ring-accent/80 focus:border-accent ${
                              form.tolerance === t
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4" role="radiogroup" aria-label="Priority Level">
                        {PRIORITIES.map((p) => {
                          ...
                          return (
                            <button
                              ...
                              className={`relative text-left p-5 border-2 transition-all duration-200 focus:outline-none focus:ring-1 focus:ring-accent/80 focus:border-accent ${
                                selected
                                  ? isBreakdown
